		...
		@for($k=0;$k<$lectures->count(); $k++)
			@if(empty($lastLecture))
					...
					<td colspan="{{$lectures[$k]->duration}}">
					 @if($lectures[$k] instanceof App\Lecture)
					 		<div class="course">Lecture: {{$lectures[$k]->course->englishName}}</div>
						   <div class="professor">Prof: {{$lectures[$k]->professor->englishName}}</div>
						   <div class="location">{{$lectures[$k]->location}}</div>
						@else
							<div class="course">Section: {{$lectures[$k]->course->englishName}}</div>
							<div class="professor">Ass: {{$lectures[$k]->assistant->englishName}}</div>
							<div class="location">{{$lectures[$k]->location}}</div>
						@endif
					</td>
					@php
